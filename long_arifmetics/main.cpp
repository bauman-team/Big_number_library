...
struct Big_number
{
	...
	{
		Big_number c;
		if (this->isNegative != b.isNegative) // —ложение не мен€ет знаки переменных => если они разные => вычитание
		{
			if (this->isNegative)
			{
	...
	{
		Big_number c;
		if (this->isNegative != b.isNegative) // ¬ычитание мен€ет знак => переменные станут одного занка => сложение
		{
		...
		{
			// «десь надо переопределить операцию вычитани€
		}
		return c;
	}
